 ...
 */
class CatBreedIdentifier(context: Context) {
    ...
    init {
        ...
        val fd = context.assets.openFd("catid_graph.tflite")
     ...
     * thread.
     */
    //fun foodPercentage(picture: Bitmap, resultCallback: (result: Float) -> Unit) {
    fun catBreedPercentage(picture: Bitmap, resultCallback: (result: Array<Float> )-> Unit) {
        ...
        executor.submit {
           ...
           // Log.i("foodP","read in data")
            val labelProbabilities = Array(1, { FloatArray(19) })
            tflite.run(imageData, labelProbabilities)
            //may require runformultipleinputsoutputs function from https://www.tensorflow.org/lite/guide/inference
            Log.i("catbreed", "ran tflite model")
            ...
            var m = 0f
            var a = 0
            for (j in 0..18){ //450 inclusive. in kotlin, unlike python, endpoint is included
                if (labelProbabilities[0][j] > m){
                    m = labelProbabilities[0][j]
                    a = j
                    Log.i("catbreed","m: $m, a: $a")
                }
            }

            resultCallback(arrayOf(a.toFloat(), m))
        }
    }
}
